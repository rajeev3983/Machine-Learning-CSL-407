...
for epoch = 1:nEpochs
    for n = 1:N
        ...
        %'TO DO'%
        for i=1:K
            deltaV(i,:) = eta*(t(1,i)-ydash(1,i))*z;
        end
        % ---------
        ...
        for i=1:H
            sm = 0;
            for k=1:K
                sm = sm + (t(k)- ydash(k))*v(k,i+1);
            end
            deltaW(i,:) = eta*sm *z(i+1)*(1- z(i+1))*curr;
        end
        v = v + deltaV;
        w = w + deltaW;
        % ---------
    ...
    trainerror(1,epoch ) = -sum(sum( Y.*log(ydash)));
    %break;
    % ---------
...
%'TO DO'%
Xdash = [ones(N,1) X];
hiddenLayerOutput = sigmoid(Xdash*(w'));
hiddenLayerOutput = [ ones(N,1) hiddenLayerOutput ];
% ---------% hidden to output for all the data points
% calculate the output of the output layer units
% ---------
%'TO DO'%
ydash = Softmax(hiddenLayerOutput*(v'));
% ---------
return;
